...
const change_board = (test)=>{
  ...
  <br>
  <input type="radio" id="option1" name="opt" value=${test[questionIdx].optn1}>
  <label for="option1">${test[questionIdx].optn1}</label>
  <br>
  <input type="radio" id="option2" name="opt" value=${test[questionIdx].optn2}>
  <label for="option2">${test[questionIdx].optn2}</label>
  <br>
  <input type="radio" id="option3" name="opt" value=${test[questionIdx].optn3}>
  <label for="option3">${test[questionIdx].optn3}</label>
  <br>
  <input type="radio" id="option4" name="opt" value=${test[questionIdx].optn4}>
...
fetch('/get-questions')
  ...
  .then(data => {
    ...
    checkAnswerBtn.addEventListener("click",() =>{
      ...
      checkIsCorrect(test[questionIdx].answer);
      questionIdx++;
    })
